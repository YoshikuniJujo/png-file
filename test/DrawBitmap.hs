	...
	case ev of
		KeyEvent {} -> do
			ch <- fmap (chr . fromEnum) $ keycodeToKeysym dpy
				(ev_keycode ev) 0
			if ch == 'q' then return () else loop' dpy win gc w h dat
		_ -> do	image dpy win gc w 0 0 0 0 dat -- $ take (2500 * 3) dat
			loop' dpy win gc w h dat
...
drawL dpy win gc filter w x0 y0 x y dat
	| null dat = return ""
	| x < w = do
		setForeground dpy gc $ rgbToWord32 $ take' 3 dat
		drawPoint dpy win gc (x0 + x) (y0 + y)
		drawL dpy win gc filter w x0 y0 (x + 1) y $ drop 3 dat
	| otherwise = return dat
...
--		print filter
		...
		dat'' <- drawL dpy win gc filter w x0 y0 x y dat'
{-
		setForeground dpy gc $ rgbToWord32 $ take' 3 dat
		drawPoint dpy win gc (x0 + x) (y0 + y)
		let (x', y') = if x == w - 1 then (0, y + 1) else (x + 1, y)
-}
		image dpy win gc w x0 y0 x (y + 1) dat'' -- $ drop 3 dat
